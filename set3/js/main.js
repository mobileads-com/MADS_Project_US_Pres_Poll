...
mads.prototype.tracker = function(tt, type, name, value) {
    ...
    if (typeof this.custTracker != 'undefined' && this.custTracker != '' && this.tracked.indexOf(name) == -1) {
        for (var i = 0; i < this.custTracker.length; i++) {
            ...
            if (name === 'int_first' && i !== 2) continue;
...
var renderAd = function() {
    ...
    var current = 0,
        pageq = [],
        results = [],
        content = 'Respond to<br>this survey once<br>to turn off all our ads';
    ...
    main.style.background = 'url(' + app.path + 'img/bg-first.jpg' + ')';
    ...
    main.addEventListener('click', function() {
        if (onceMain && !showSlide) {
            ...
            onceMain = false;
            app.tracker('E', 'participatenow_respond');
        }

        if (showSlide) {
          app.tracker('E', 'int_first');
            showSlide = false;
